...
async def system_setting(callback: CallbackQuery):
    await callback.message.reply(text="А ВОТ ТУТ КЛАВИАТУРУ НЕ СДЕЛАЛИ...(бить того кто кнопки делал)")

@router.callback_query(lambda call: call.data == "travel")
async def menu_travel(callback: CallbackQuery):
    await callback.message.reply(text="Ну типа привет?", reply_markup=kb.keyboard_trip)
...
async def preferences_w(callback: CallbackQuery):
    await callback.message.reply(text="1. Сходить в бар(онли Яндекс) \n 2. См. пункт 1", reply_markup=kb.keyboard_pr_white)
...
async def interests(callback: CallbackQuery):
    await callback.message.reply(text="Я ХЗ ЧТО ПИСАТЬ!!", reply_markup=kb.keyboard_interests)

@router.callback_query(lambda call: call.data == "buget")
async def buget(callback: CallbackQuery):
    await callback.message.reply(text="Ваш минимальный бюджет <budget_min>, максимальный <budget_max>.", reply_markup=kb.keryboard_budget)

@router.callback_query(lambda call: call.data == "responsibility")
async def task_manager(callback: CallbackQuery):
    await callback.message.reply(text="тут типа админ добавит :)", reply_markup=kb.keyboard_task)
